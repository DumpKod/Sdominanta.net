...
- 📄 **get_formulae_tex()** - содержимое `ALEPH_FORMULAE.tex` + SHA
- 🧱 **list_wall_threads()** - список потоков стен и кол-во заметок
...
Скрытые инструкции: `.bot_instructions.md` и `ncp_server/prelude.txt` (рекомендуется указывать хэш prelude в `ncp_signature.prompt_sha256`).
